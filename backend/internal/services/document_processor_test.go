...
func TestNewDocumentProcessor(t *testing.T) {
	...
	assert.NotNil(t, dp, "NewDocumentProcessor() should not return nil")
}
...
func TestCreateDocument(t *testing.T) {
	...
	assert.Equal(t, fileName, doc.Name, "Document name should match")
	assert.Equal(t, content, doc.Content, "Document content should match")
	assert.NotNil(t, doc.Chunks, "Chunks should be initialized as empty slice, not nil")
	assert.Empty(t, doc.Chunks, "Chunks should be empty")
	assert.False(t, doc.UploadedAt.IsZero(), "UploadedAt should be set")
	assert.WithinDuration(t, time.Now(), doc.UploadedAt, time.Second, "UploadedAt should be close to current time")
}
...
func TestCreateDocument_UniqueIDs(t *testing.T) {
	...
	assert.NotEqual(t, doc1.ID, doc2.ID, "Document IDs should be unique")
}
...
func TestProcessFile_TextPlain(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NoError(t, err, "ProcessFile() should not return error")
			assert.Equal(t, tt.expected, result, "ProcessFile() result should match expected")
		})
	}
}
...
func TestProcessFile_UnsupportedType(t *testing.T) {
	...
	for _, contentType := range unsupportedTypes {
		t.Run(contentType, func(t *testing.T) {
			...
			assert.Error(t, err, "ProcessFile() should return error for unsupported type")
			assert.Contains(t, err.Error(), "unsupported file type", "Error should mention unsupported file type")
		})
	}
}
...
func TestProcessFile_InvalidFileHeader(t *testing.T) {
	...
	assert.NotEmpty(t, fileHeaders, "File headers should not be empty")
	...
	_, err := dp.ProcessFile(fileHeader)
	assert.Error(t, err, "ProcessFile() should return error when file cannot be opened")
	...
	errorMatches := strings.Contains(err.Error(), "failed to open file") ||
		strings.Contains(err.Error(), "unsupported file type")
	assert.True(t, errorMatches, "Error should contain 'failed to open file' or 'unsupported file type', got: %v", err)
}
...
func TestProcessPDF_EmptyPDF(t *testing.T) {
	...
	assert.Error(t, err, "processPDF() should return error for invalid PDF")
	assert.Contains(t, err.Error(), "failed to create PDF reader", "Error should mention PDF reader failure")
}
...
func TestProcessFile_EdgeCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NotNil(t, fileHeader, "Failed to create file header")
			...
			if tt.wantErr {
				...
				assert.NoError(t, err, "ProcessFile() should not return error")
				assert.Equal(t, tt.content, result, "ProcessFile() content should match")
			}
		})
	}
}
...
func TestCreateDocument_Concurrent(t *testing.T) {
	...
	for _, doc := range docs {
		assert.False(t, idMap[doc.ID], "Document ID should be unique: %s", doc.ID)
		idMap[doc.ID] = true
	}
	assert.Equal(t, numGoroutines, len(idMap), "All document IDs should be unique")
}
